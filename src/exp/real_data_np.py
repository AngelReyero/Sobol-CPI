...
def main(args):
    ...
    # Results storage
    # ---------------------------------------------------------
    importance_score = np.zeros((26, len(args.corr), p+1))   # +1 for new feature
    p_val = np.zeros((26, len(args.corr), p+1))
    exec_time = np.zeros((26, len(args.corr)))
    ...
    csv_path = os.path.abspath(os.path.join(os.path.dirname(__file__), f"../../results/csv/realdata/{args.data}_{args.model}_np_seed{s}.csv"))
    f_res.to_csv(
        csv_path,
        index=False,
        ) 
