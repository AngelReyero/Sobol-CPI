import numpy as np
import vimpy
from sobol_CPI_np import Sobol_CPI
...
def main(args):
    ...
    # Results storage
    # ---------------------------------------------------------
    importance_score = np.zeros((28, len(args.corr), p+1))   # +1 for new feature
    p_val = np.zeros((28, len(args.corr), p+1))
    exec_time = np.zeros((28, len(args.corr)))
    r2 = np.zeros((len(args.corr)))
    ...
    # 0 LOCO-W, 1-6 Sobol-CPI(1) (-, sqrt, n, bootstrap, n2), 7-11 Sobol-CPI(10)(-, sqrt, n, bootstrap, n2), 12-16 Sobol-CPI(100) (-, sqrt, n, bootstrap, n2), 17-21 LOCO (-, sqrt, n, bootstrap, n2)
        ...
        for idx, ptype in enumerate(['sign_test','wilcox']):
            start_time = time.time()
            p_val[21+idx, :] = sobol_CPI.score(X_test, y_test, n_cal=1, p_val=ptype)["pval"].reshape(p)
            exec_time[21+idx] = time.time() - start_time + imp_time + tr_time
        ...
        exec_time[25, i] = time.time() - start_time + tr_time
        importance_score[25,i]= pi_importance["importance"].reshape((p,))
        ...
        for idx, ptype in enumerate(['sign_test','wilcox']):
            start_time = time.time()
            p_val[23+idx, :] = loco.score(X_test, y_test, p_val=ptype)["pval"].reshape(p)
            exec_time[23+idx] = time.time() - start_time + tr_loco_time + tr_time 
        ...
        time_samp = time.time() - time_samp
        # CFI
        start_time = time.time()
        cfi = wrk.cfi(X_test_df, y_test_df)
        cfi_scores = cfi.fi_means_stds()
        importance_score[26, i]= np.array([
            cfi_scores[k]*0.5 for k in cfi_scores.index if k != 'std'
        ])
        exec_time[26, i] = time.time() - start_time + tr_time + time_samp
        ...
        importance_score[27, i]= np.array([
            scSAGEvfj_scores[k] for k in scSAGEvfj_scores.index if k != 'std'
        ])
        exec_time[27, i] = time.time() - start_time + tr_time + time_samp
    ...
    for i in range(28):
        for j in range(len(args.corr)):
            ...
            if i==0:
                ...
                f_res1["method"] = ["CPI_ST"]
            elif i==22:
                f_res1["method"] = ["CPI_wilcox"]
            elif i==23:
    ...
    csv_path = os.path.abspath(os.path.join(os.path.dirname(__file__), f"../../results/csv/realdata/ext_{args.data}_{args.model}_np_seed{s}.csv"))
    f_res.to_csv(
        csv_path,
        index=False,
        ) 
